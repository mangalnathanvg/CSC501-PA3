...
struct	pentry	{
	...
	int pinh;			/* Current Inherited Priority */
	int lockid;			/* Lock descriptor in whose wait queue the process is blocked */
	int locks[50];		/* Bit mask of all the locks held by the process */
	
	int ltype;			/* The type of lock (READ or WRITE) the process is currently waiting for */	
	int plockret;       /* Type returned when process calls lock() method */
	unsigned long waittime; /* The time for which the process is waiting for the lock */
};
